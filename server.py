...
def is_repeated_noise_input(question: str) -> bool:   #bug(27): 반복 문자열 방지
    compact_text = "".join(question.strip().split())

    if len(compact_text) < 6:
        return False

    chars = list(compact_text)
    ...
    most_common_ratio = most_common_count / total_length

    if most_common_ratio >= 0.8:
        return True

    max_pattern_size = min(3, total_length // 2)

    for pattern_size in range(1, max_pattern_size + 1):
        if total_length % pattern_size != 0:
            continue

        pattern = compact_text[:pattern_size]
        repeat_count = total_length // pattern_size

        if repeat_count >= 3 and pattern * repeat_count == compact_text:
            return True
...
def classify_current_input(question: str) -> str:
    ...
    """
    if is_repeated_noise_input(question):
